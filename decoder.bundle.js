...
class OmvDecodedTileEmitter {
    constructor(m_decodeInfo, m_styleSetEvaluator, m_gatherFeatureIds, m_skipShortLabels, m_enableElevationOverlay, m_languages) {
     ...
     */
    processPointFeature(layer, extents, geometry, env, techniques, featureId) {
     ...
     * @param featureId The id of the feature.
     */
    processLineFeature(layer, extents, geometry, env, techniques, featureId) {
        this.processFeatureCommon(env);
        const lines = [];
        const uvs = [];
        ...
        for (const technique of techniques) {
            ...
            if (!computeTexCoords) {
                ...
                harp_utils_1.assert(texCoordinateType === this.getTextureCoordinateType(technique));
            }
        }
        ...
        for (const technique of techniques) {
            ...
            if (techniqueName === "line" ||
                ...
                this.applyLineTechnique(lineGeometry, technique, techniqueIndex, this.m_gatherFeatureIds, lineType, featureId, lines, env, this.getTextureCoordinateType(technique) ? uvs : undefined);
            }
                ...
                const { positions, indices, groups, featureIds, featureStarts, objInfos } = meshBuffers;
     ...
     * @param featureId The id of the feature.
     */
    processPolygonFeature(layer, extents, geometry, env, techniques, featureId) {
        this.processFeatureCommon(env);
        techniques.forEach(technique => {
            ...
            if (isPolygon) {
                this.applyPolygonTechnique(polygons, technique, techniqueIndex, featureId, env, extents);
            }
            else if (isLine) {
    ...
    getComputeTexCoordsFunc(technique) {
        ...
        return texCoordType === harp_datasource_protocol_1.TextureCoordinateType.TileSpace
            ...
            : texCoordType === harp_datasource_protocol_1.TextureCoordinateType.EquirectangularSpace
                ? (tilePos, extents) => {
                    const worldPos = OmvUtils_1.tile2world(extents, this.m_decodeInfo, tilePos);
    ...
    applyLineTechnique(linesGeometry, technique, techniqueIndex, gatherFeatureIds, lineType = LineType.Complex, featureId, lines, env, uvs) {
    ...
    applyPolygonTechnique(polygons, technique, techniqueIndex, featureId, env, extents) {
        const isExtruded = harp_datasource_protocol_1.isExtrudedPolygonTechnique(technique);
        ...
        const color = new THREE.Color(extrudedPolygonTechnique.color !== undefined
            ? harp_datasource_protocol_1.getPropertyValue(extrudedPolygonTechnique.color, this.m_decodeInfo.tileKey.level)
            : this.isColorStringValid(env.lookup("color"))
                ? env.lookup("color")
                : extrudedPolygonTechnique.defaultColor !== undefined
                    ? harp_datasource_protocol_1.getPropertyValue(extrudedPolygonTechnique.defaultColor, this.m_decodeInfo.tileKey.level)
                    : 0x000000);
                ...
                try {
                    ...
                    if (isSpherical) {
                        ...
                        for (let i = 0; i < vertices.length; i += vertexStride) {
                            const worldPos = OmvUtils_1.tile2world(extents, this.m_decodeInfo, tmpV2.set(vertices[i], vertices[i + 1]), true);
                        ...
                        for (let i = 0; i < posAttr.array.length; i += 3) {
                            const tilePos = OmvUtils_1.world2tile(extents, this.m_decodeInfo, tmpV2.set(posAttr.array[i], posAttr.array[i + 1]), true);
                        ...
                        triangles.push(...geom.getIndex().array);
                    }
...
exports.Ring = Ring;
class OmvDecoder {
    constructor(m_projection, m_styleSetEvaluator, m_showMissingTechniques, m_dataFilter, m_featureModifier, m_gatherFeatureIds = true, m_createTileInfo = false, m_gatherRoadSegments = false, m_skipShortLabels = true, m_storageLevelOffset = 0, m_enableElevationOverlay = false, m_languages) {
        ...
        this.m_dataAdapters.push(new OmvData_1.OmvProtobufDataAdapter(this, m_dataFilter, logger));
        this.m_dataAdapters.push(new VTJsonDataAdapter_1.VTJsonDataAdapter(this, m_dataFilter, logger));
    }
    /**
    ...
    getDecodedTile(tileKey, data) {
        ...
        this.m_decodedTileEmitter = new OmvDecodedTileEmitter_1.OmvDecodedTileEmitter(decodeInfo, this.m_styleSetEvaluator, this.m_gatherFeatureIds, this.m_skipShortLabels, this.m_enableElevationOverlay, this.m_languages);
    ...
    processPointFeature(layer, extents, geometry, env, storageLevel) {
        ...
        const techniques = this.applyKindFilter(this.m_styleSetEvaluator.getMatchingTechniques(env), harp_datasource_protocol_1.GeometryKind.Label);
        ...
        if (this.m_decodedTileEmitter) {
            this.m_decodedTileEmitter.processPointFeature(layer, extents, geometry, env, techniques, featureId);
        }
        if (this.m_infoTileEmitter) {
            this.m_infoTileEmitter.processPointFeature(layer, extents, geometry, env, techniques, featureId);
        }
    }
    processLineFeature(layer, extents, geometry, env, storageLevel) {
        ...
        const techniques = this.applyKindFilter(this.m_styleSetEvaluator.getMatchingTechniques(env), harp_datasource_protocol_1.GeometryKind.Line);
        if (techniques.length === 0) {
            if (this.m_showMissingTechniques) {
        ...
        const featureId = env.lookup("$id");
        if (this.m_decodedTileEmitter) {
            this.m_decodedTileEmitter.processLineFeature(layer, extents, geometry, env, techniques, featureId);
        }
        if (this.m_infoTileEmitter) {
            this.m_infoTileEmitter.processLineFeature(layer, extents, geometry, env, techniques, featureId);
        }
    }
    processPolygonFeature(layer, extents, geometry, env, storageLevel) {
        ...
        const techniques = this.applyKindFilter(this.m_styleSetEvaluator.getMatchingTechniques(env), harp_datasource_protocol_1.GeometryKind.Area);
        if (techniques.length === 0) {
            if (this.m_showMissingTechniques) {
        ...
        const featureId = env.lookup("$id");
        if (this.m_decodedTileEmitter) {
            this.m_decodedTileEmitter.processPolygonFeature(layer, extents, geometry, env, techniques, featureId);
        }
        if (this.m_infoTileEmitter) {
            this.m_infoTileEmitter.processPolygonFeature(layer, extents, geometry, env, techniques, featureId);
        }
    }
    /**
...
(function (OmvDecoder) {
    class DecodeInfo {
        ...
        constructor(targetProjection, tileKey, tileSizeOnScreen) {
            ...
            this.projectedBoundingBox = new harp_geometry_1.OrientedBox3();
            /**
...
class OmvTileDecoder extends index_worker_1.ThemedTileDecoder {
    ...
    decodeThemedTile(data, tileKey, styleSetEvaluator, projection) {
        ...
        const decoder = new OmvDecoder(projection, styleSetEvaluator, this.m_showMissingTechniques, this.m_featureFilter, this.m_featureModifier, this.m_gatherFeatureIds, this.m_createTileInfo, this.m_gatherRoadSegments, this.m_skipShortLabels, this.m_storageLevelOffset, this.m_enableElevationOverlay, this.languages);
...
class OmvTileInfoEmitter {
    ...
    processPointFeature(layer, extents, geometry, env, techniques, featureId) {
        ...
        for (const technique of techniques) {
            ...
            for (const pos of geometry) {
                ...
                tileInfoWriter.addFeature(this.m_tileInfo.pointGroup, technique, env, featureId, infoTileTechniqueIndex, harp_datasource_protocol_1.FeatureGroupType.Point);
    ...
    processLineFeature(layer, extents, geometry, env, techniques, featureId) {
        ...
        for (const technique of techniques) {
            ...
            const infoTileTechniqueIndex = tileInfoWriter.addTechnique(technique);
            for (const aLine of lines) {
                tileInfoWriter.addFeature(this.m_tileInfo.lineGroup, technique, env, featureId, infoTileTechniqueIndex, harp_datasource_protocol_1.FeatureGroupType.Line);
    ...
    processPolygonFeature(layer, extents, geometry, env, techniques, featureId) {
        ...
        for (const technique of techniques) {
            ...
            const infoTileTechniqueIndex = tileInfoWriter.addTechnique(technique);
            tileInfoWriter.addFeature(this.m_tileInfo.polygonGroup, technique, env, featureId, infoTileTechniqueIndex, harp_datasource_protocol_1.FeatureGroupType.Polygon);
        }
...
class VTJsonDataAdapter {
    ...
    process(tile, tileKey, geoBox) {
        for (const feature of tile.features) {
            const env = new index_decoder_1.MapEnv(Object.assign({}, feature.tags, { $layer: tile.layer, $geometryType: this.convertGeometryType(feature.type), $level: tileKey.level, id: feature.id }));
            switch (feature.type) {
